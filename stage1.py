...
def ebsynth_utility_stage1(dbg, project_args, auto_pick_key_frame, selected_frame_type, frame_width, frame_height, frame_wh_scale, key_min_gap, key_max_gap, key_th, key_add_last_frame):
    ...
    if not os.listdir(tmp_key_frame):
        subprocess.call(['ffmpeg', '-i', video_file, '-qscale:v', '0', '-vf',
        ...
        for i, frame_name in enumerate(frame_names_subset):
            ...
            if frame.shape == key.shape:
                ...
                if diff == 0:
                    ...
                    # cv2.imwrite(os.path.join(video_frame, frame_name), frame)
                    shutil.copy(os.path.join(frames_dir, frame_name),os.path.join(video_frame, frame_name)) 
                    break
